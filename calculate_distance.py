"""
Detailed documentation for this module's functions.
...
#*numeric_distance(u, v, w_rooms=0.2, w_mates=0.3, w_budget=0.35, w_months=0.15):
...
def norm_rooms(a, b):
    """Compute a normalized distance between desired room counts."""
    return min(abs(a-b), 2) / 2.0

def norm_roommates(a, b):
    """Compute a normalized distance between desired roommate counts."""
    return min(abs(a-b), 2) / 2.0

def norm_budget(a, b):
    """Compute a scale-invariant budget distance in [0, 1]."""
...
def norm_months(a, b, cap=36):
    """Compute a normalized duration distance after clamping to [1, cap]."""
    a = max(1, min(a, cap))
    b = max(1, min(b, cap))
    return abs(a-b) / cap

def numeric_distance(u, v, w_rooms=0.2, w_mates=0.3, w_budget=0.35, w_months=0.15):
    """Aggregate a weighted numeric distance across preference dimensions."""
    d = 0.0
